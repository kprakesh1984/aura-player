...
contextBridge.exposeInMainWorld('electronAPI', {
    ...
    onContextMenuCommand: (callback) => 
        ipcRenderer.on('context-menu-command', (_event, command, data) => callback(command, data)),

    // Window Management and Playlist State Persistence
    resizeMainWindowAndSaveVisibility: (width, heightOption, isPlaylistVisible) => 
        ipcRenderer.send('set-main-window-size-and-visibility', width, heightOption, isPlaylistVisible),
    onSetInitialPlaylistVisibility: (callback) => 
        ipcRenderer.on('set-initial-playlist-visibility', (_event, isVisible) => callback(isVisible)),
    ...
    onOpenFileInPlayer: (callback) => 
        ipcRenderer.on('open-file-in-player', (_event, filePath) => callback(filePath)),
});