...
contextBridge.exposeInMainWorld("playlistAPI", {
  ...
  onInitializePlaylist: (callback) =>
    ipcRenderer.on("initialize-playlist", (_event, ...args) =>
      callback(...args)
    ),
  onUpdatePlaylistData: (callback) =>
    ipcRenderer.on("update-playlist-data", (_event, ...args) =>
      callback(...args)
    ),

  // Send actions to main process (which will relay to main player window)
  sendActionToMain: (action, data) =>
    ipcRenderer.send("playlist-action", action, data), // Generic action dispatcher

  // Context Menu
  showContextMenu: (trackIndex, selectedIndices) =>
    ipcRenderer.send("show-playlist-context-menu", trackIndex, selectedIndices),
  onContextMenuCommand: (callback) =>
    ipcRenderer.on("context-menu-command", (_event, ...args) =>
      callback(...args)
    ),

  // Sync actions from main player
  onSyncPlaylistAction: (callback) =>
    ipcRenderer.on("sync-playlist-action", (_event, ...args) =>
      callback(...args)
    ),
