...
function handleOpenFile(filePath) {
  ...
  if (!supportedExtensions.includes(fileExt)) {
    console.log(
      `Main: handleOpenFile - Path "${filePath}" (ext: ${fileExt}) is not a supported audio file type. Ignoring.`
    );
    return;
  }
  try {
    if (!fs.existsSync(filePath) || !fs.lstatSync(filePath).isFile()) {
      console.warn(
        `Main: handleOpenFile - Path "${filePath}" does not exist or is not a file. Ignoring.`
      );
      return;
    }
  } catch (err) {
    console.error(
      `Main: handleOpenFile - Error checking file status for "${filePath}":`,
      err
    );
    return;
  }
  ...
  if (
    ...
    console.log(
      'Main: handleOpenFile - Sending "open-file-in-player" IPC directly for:',
      filePath
    );
    ...
    console.log(
      "Main: handleOpenFile - mainWindow not fully ready or webContents loading, queueing file:",
      filePath
    );
...
if (!gotTheLock) {
  ...
  app.on("second-instance", (event, commandLine, workingDirectory) => {
    console.log(
      "Main: 'second-instance' event triggered on PRIMARY instance. Full CommandLine:",
      commandLine
    );
    if (mainWindow) {
      if (mainWindow.isMinimized()) mainWindow.restore();
    ...
    for (let i = 1; i < commandLine.length; i++) {
      ...
      if (!arg.startsWith("--") && arg !== ".") {
        ...
        if (supportedExtensions.includes(ext)) {
          ...
          filePath = arg;
          console.log(
            `Main (second-instance): Potential file path found: "${filePath}"`
          );
          break;
        }
      }
    ...
    if (filePath) {
      console.log(
        "Main: second-instance successfully determined file to open:",
        filePath
      );
      handleOpenFile(filePath); // This will queue if necessary or send IPC
    } else {
      console.warn(
        "Main: second-instance - no valid audio file path found in commandLine:",
        commandLine
      );
    }
  });
...
function createMainWindow() {
  ...
  mainWindow.webContents.on("did-finish-load", () => {
    ...
    if (mainWindow && mainWindow.webContents && !mainWindow.isDestroyed()) {
      ...
      console.log(
        "Main (did-finish-load): Sending initial states to renderer."
      );
      ...
      console.log(
        "Main (did-finish-load): Raw process.argv for initial launch:",
        process.argv
      );
      ...
      if (filePathFromArg) {
        console.log(
          "Main (did-finish-load): Initial launch argument detected file:",
          filePathFromArg
        );
      ...
      if (filesToProcessNow.length > 0) {
        console.log(
          "Main (did-finish-load): Processing files to open via direct IPC send:",
          filesToProcessNow
        );
        filesToProcessNow.forEach((fp) => {
          ...
          console.log(
            'Main (did-finish-load): Directly sending "open-file-in-player" for:',
            fp
          );
          ...
          if (supportedExtensions.includes(fileExt)) {
            try {
              if (fs.existsSync(fp) && fs.lstatSync(fp).isFile()) {
                ...
                console.warn(
                  `Main (did-finish-load): Path "${fp}" does not exist or is not a file. Not sending.`
                );
              }
            } catch (err) {
              console.error(
                `Main (did-finish-load): Error checking file status for "${fp}":`,
                err
              );
            }
          } else {
            console.log(
              `Main (did-finish-load): Path "${fp}" (ext: ${fileExt}) is not supported. Not sending.`
            );
          }
...
ipcMain.handle("dialog:openFiles", async () => {
  ...
  if (!mainWindow) {
    console.error(
      "Main IPC Error: 'dialog:openFiles' called but mainWindow is not defined."
    );
    return [];
  }
  try {
    ...
    if (canceled || !filePaths || filePaths.length === 0) {
      console.log(
        "Main IPC: File open dialog was cancelled or no files selected."
      );
      return [];
    }
    console.log("Main IPC: Files selected from dialog:", filePaths);
    return filePaths;
  } catch (error) {
    console.error(
      "Main IPC Error in showOpenDialog for 'dialog:openFiles':",
      error
    );
    return [];
  }
...
ipcMain.on(
  ...
  (event, newWidthIgnored, heightOption, isPlaylistNowVisible) => {
    console.log(
      "Main IPC: 'set-main-window-size-and-visibility' called. HeightOpt:",
      heightOption,
      "Visible:",
    ...
    if (mainWindow) {
      ...
      if (heightOption === "player_with_playlist") {
        ...
        targetContentHeight = MAIN_PLAYER_CONTENT_HEIGHT;
      }
