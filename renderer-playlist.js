document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPlaylist() {
        ...
        if (playlist.length === 0) {
            ...
            li.textContent = 'Playlist is empty. Add songs from the main player or drag here.';
    ...
    function handlePlaylistItemClick(event, index) {
        ...
        const selectionChanged = oldSelectedIndices.length !== selectedIndices.length || oldSelectedIndices.some(idx => !selectedIndices.includes(idx));
        if (selectionChanged && !(event.ctrlKey || event.metaKey) && !event.shiftKey) {
    ...
    playlistArea.addEventListener('dragover', (e) => {
        ...
        playlistUL.querySelector('.playlist-drop-target')?.classList.add('dragover');
        if (!playlistUL.querySelector('.playlist-drop-target') && playlist.length > 0) {
            playlistUL.classList.add('dragover'); // Add to whole list if not empty
        }
    });
    playlistArea.addEventListener('dragleave', (e) => {
        e.preventDefault(); e.stopPropagation();
        playlistUL.querySelector('.playlist-drop-target.dragover')?.classList.remove('dragover');
    ...
    playlistArea.addEventListener('drop', async (e) => {
        ...
        if (files.length > 0) {
            ...
            if (audioFilePaths.length > 0) {
                ...
                window.playlistAPI.sendActionToMain('add-files', { filePaths: audioFilePaths });
    ...
    window.playlistAPI.onInitializePlaylist((initialPlaylist, initialIndex) => {
        console.log('Detached Playlist Initializing:', initialPlaylist, initialIndex);
    ...
    window.playlistAPI.onUpdatePlaylistData((updatedPlaylist, updatedIndex) => {
        console.log('Detached Playlist Data Update:', updatedPlaylist, updatedIndex);
        ...
        if (currentTrackIndex > -1 && !selectedIndices.includes(currentTrackIndex)) {
            // If current track changed and is not part of selection, make it selected.
            selectedIndices = [currentTrackIndex];
        }
        lastClickedIndex = currentTrackIndex > -1 ? currentTrackIndex : lastClickedIndex;
        renderPlaylist();
    });
